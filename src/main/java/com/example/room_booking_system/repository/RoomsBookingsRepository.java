...
public interface RoomsBookingsRepository extends JpaRepository<RoomsBookings, Long> {
    @Query("SELECT r FROM RoomsBookings r WHERE r.roomId = :roomId AND  r.bookingDate = :bookingDate ORDER BY r.timeFrom ASC")
    List<RoomsBookings> findAllByRoomIdAndBookingDate(Long roomId, LocalDate bookingDate);

    @Query("SELECT count(rb.roomId) FROM RoomsBookings rb WHERE rb.roomId = :roomId AND rb.email = :email AND rb.bookingDate = :bookingDate")
    Long findRoomBookingByRoomIdEmailAndDate(Optional<Long> roomId, String email, LocalDate bookingDate);

    @Query("SELECT rb FROM RoomsBookings rb WHERE rb.roomId = :roomId AND rb.bookingDate = :bookingDate " +
    ...
    @Query("SELECT rb FROM RoomsBookings rb WHERE rb.roomId = :roomId AND rb.email = :email AND rb.bookingDate = :bookingDate ")
